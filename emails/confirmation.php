...
function wpwa_paddle_send_trialing_email_by_sub_id($sub_id) {
    global $wpdb;
    ...
    $customer = wpwa_paddle_get_customer_by_weebly_id($subscription['weebly_user_id']);
    
    if (!$product || !$customer) return false;

    $to = $customer['email'];
    $subject = sprintf('[%s] Your Free Trial Has Started! - %s', get_bloginfo('name'), $product['name']);
    
    $message = wpwa_paddle_get_trialing_email_html(array(
...
function wpwa_paddle_get_trialing_email_html($data) {
    $subscription = $data['subscription'];
    $product = $data['product'];
    $customer = $data['customer'];
    
    $trial_end_date = date('F j, Y', strtotime($subscription['current_period_end']));
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; background-color: #f4f7fa;">
    <div class="email-wrapper" style="max-width: 600px; margin: 40px auto; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.08);">
        ...
        <div class="header" style="background: linear-gradient(135deg, #4fd1c5 0%, #38b2ac 100%); color: #fff; padding: 40px 40px 35px; text-align: center;">
            <h1 style="margin: 0 0 8px; font-size: 32px; font-weight: 700; line-height: 1.2; color: #ffffff;">✨ Trial Activated!</h1>
            <p style="margin: 0; font-size: 16px; opacity: 0.95; color: #ffffff;">Enjoy full access to <?php echo esc_html($product['name']); ?></p>
        </div>
        
        <div class="content" style="padding: 35px 40px 25px;">
            <p style="margin: 0 0 15px; font-size: 16px; color: #2d3748; line-height: 1.6;">Hi <strong><?php echo esc_html($customer['name'] ?: 'there'); ?></strong>,</p>
            
            <p style="margin: 0 0 15px; font-size: 16px; color: #2d3748; line-height: 1.6;">Your free trial for <strong><?php echo esc_html($product['name']); ?></strong> is now officially active. You have full access to all premium features starting right now.</p>
            
            <div style="background: #fffaf0; padding: 20px 25px; border-radius: 10px; margin: 20px 0 30px; border: 1px solid #feebc8;">
                <h3 style="margin: 0 0 15px; font-size: 18px; font-weight: 700; color: #744210;">⏳ Trial Details</h3>
                
                <div style="display: table; width: 100%;">
                    <div style="display: table-row;">
                        <div style="display: table-cell; padding: 6px 0; width: 40%; font-size: 13px; color: #7b341e; font-weight: 600;">Status</div>
                        <div style="display: table-cell; padding: 6px 0; width: 60%; font-size: 13px; color: #2d3748; font-weight: 600;">Free Trial</div>
                    </div>
                    <div style="display: table-row;">
                        <div style="display: table-cell; padding: 6px 0; width: 40%; font-size: 13px; color: #7b341e; font-weight: 600;">Trial Ends On</div>
                        <div style="display: table-cell; padding: 6px 0; width: 60%; font-size: 13px; color: #2d3748; font-weight: 600;"><?php echo $trial_end_date; ?></div>
                    </div>
                </div>
                <p style="margin: 15px 0 0; font-size: 12px; color: #7b341e; line-height: 1.4;">Note: You won't be charged until the end of your trial. You can manage your subscription at any time from your dashboard.</p>
            </div>
            
            <p style="margin: 0 0 15px; font-size: 16px; color: #2d3748; line-height: 1.6;"><strong>Get Started:</strong></p>
            <p style="margin: 0 0 15px; font-size: 16px; color: #2d3748; line-height: 1.6;">Simply head back to your Weebly Editor to start using the app. No extra configuration is required!</p>
        </div>

        <div class="content" style="padding: 0 40px 35px;">
            <div style="background-color: #f7fafc; border-left: 4px solid #4fd1c5; padding: 15px 18px; border-radius: 6px;">
                <p style="margin: 0 0 8px; font-size: 14px; color: #2d3748; line-height: 1.5; font-style: italic;">"I hope you enjoy exploring the features of <?php echo esc_html($product['name']); ?>. If you have any feedback or need help during your trial, just reply to this email!"</p>
                <p style="margin: 0; font-size: 13px; color: #4a5568; font-weight: 600; font-style: normal;">— Junaid Hassan, Developer</p>
            </div>
        </div>
        
        <div style="padding: 25px 40px; background: #1a202c; text-align: center;">
            <p style="margin: 0 0 12px; font-size: 14px; color: #ffffff; font-weight: 700;">Codoplex</p>
            <p style="margin: 0 0 8px; font-size: 12px; color: #a0aec0;">
                <a href="https://codoplex.com" style="color: #90cdf4; text-decoration: none;">Website</a> |
                <a href="https://codoplex.com/contact" style="color: #90cdf4; text-decoration: none;">Support</a>
            </p>
        </div>
    </div>
</body>
